...
print(f"Побитовое «ИЛИ» для {bin(n1)} и {bin(n2)}: {bin(bit_or)} ({bit_or})")

print(f"Исключающее «ИЛИ» для {bin(n1)} и {bin(n2)}: {bin(bit_xor)} ({bit_xor})")

print(f"Побитовое «И» для {bin(n1)} и {bin(n2)}: {bin(bit_and)} ({bit_and})")

print(f"Побитовое отрицание для {bin(n1)}: {bin(bit_not)} ({bit_not})")
